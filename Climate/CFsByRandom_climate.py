...
exp_random_climate = Dice(d_climate,m_climate,method="random")
...
generate_failure = exp_random_climate.generate_counterfactuals(query_instances_failure,total_CFs=100,desired_class=0)
final_failure_cfs = [None]*n_cfs
for i in range(n_cfs):
    final_failure_cfs[i] = tuple(generate_failure.cf_examples_list[0].final_cfs_df.values[i])
    pass
generate_success = exp_random_climate.generate_counterfactuals(query_instances_success,total_CFs=100,desired_class=1)
...
ClimateCFs.to_csv("ClimateCFs-Random.csv",index=True)